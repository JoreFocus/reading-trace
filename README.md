...
如果你不会写代码，尤其不要让项目状态只留在当前对话里。这个仓库提供了 [`HANDOFF.md`](HANDOFF.md)，用来记录已完成内容、已知问题和下一步。

每完成一个能够正常运行的阶段，让 Agent 做两件事：

1. 运行测试并提交一个 Git 检查点；
2. 更新 `HANDOFF.md`，使下一个 Agent 不依赖旧对话也能恢复工作。

Git 保存“哪个版本还能运行”，`HANDOFF.md` 说明“现在做到了哪里”。两者一起使项目可以暂停、换对话后继续，也能在出错时回到最近的稳定状态。
